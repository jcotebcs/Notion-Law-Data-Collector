...
It also includes functionality to search CourtListener API for case information with enhanced fields.
...
CourtListener Search Enhanced Fields:
- Citation: Case citations
- Author: Judge or author of the opinion
- Case Type: CourtListener case type
- Status: Procedural status
- Attorney: Attorneys involved
- Summary: Brief case summary

Usage:
python notion_law_data_collector_script.py
"""
...
def search_courtlistener_cases(case_name, api_key):
    ...
    Returns the first 5 relevant results with additional fields or None if no API key.
    """
    if not api_key:
        return None
    ...
    params = {
        ...
        'type': 'o',  # opinions
        'format': 'json',
        'fields': 'caseName,dateFiled,court,docketNumber,absolute_url,cluster,citation,author,caseType,status,attorney,summary',
        'page_size': 5
    }
    headers = {
        ...
        if 'results' in data:
            ...
            for result in data['results']:
                processed_result = {
                    ...
                    'citation': result.get('citation', [''])[0] if isinstance(result.get('citation'), list) and result.get('citation') else result.get('citation', ''),
